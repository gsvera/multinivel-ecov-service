...
public interface PayAffiliateRepository extends JpaRepository<PayAffiliate, Long> {
    @Query(value = "SELECT pa.id, pa.amount, pa.created_date, pa.description, pa.status_pay, pa.payment_file ,pa.pay_method,\n" +
            ...
            "FROM tbl_pay_affiliate AS pa LEFT JOIN tbl_users AS u ON pa.id_user = u.id WHERE (?1 IS NULL OR (SELECT concat(first_name, ' ',last_name, ' ', email) FROM tbl_users AS u WHERE u.id = pa.id_user) ILIKE %?1%) ORDER BY pa.id;", nativeQuery = true)
