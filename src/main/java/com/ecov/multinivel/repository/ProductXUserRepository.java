...
public interface ProductXUserRepository extends JpaRepository<ProductXUser, Long> {
    ...
    @Query(value = "SELECT pu.id, p.name, pu.date_buy, pu.payment_file, pu.pay_method, pu.id_pay_affiliate, \n" +
            "(SELECT name_status FROM tbl_status_buy AS sb WHERE sb.id = pu.id_status_buy),\n" +
            ...
            "FROM tbl_product_x_user AS pu LEFT JOIN tbl_product AS p ON pu.id_product = p.id WHERE (?1 IS NULL OR (SELECT concat(first_name, ' ',last_name) FROM tbl_users AS u WHERE u.id = pu.id_user) ILIKE %?1%) ORDER BY pu.id", nativeQuery = true)
    Page<Object[]> findProductByFilter(String word, Pageable pageable);
    Optional<ProductXUser> findByIdPayAffiliate(Long id);
}
